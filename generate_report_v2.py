...
output_file = f"weibo_analysis_report_{today_str}.html"

# The content is hardcoded based on the agent's analysis of the provided JSON data.
# In a fully automated pipeline, this would be generated by an LLM call.
# Since I am the LLM, I am generating the content here and embedding it.

report_data = {
    "date": datetime.datetime.now().strftime("%Y/%m/%d"),
    "report_title": "微博热搜深度分析",
    "subtitle": "深度挖掘社交媒体趋势 · 洞察算法挑战权威 · 重塑底层生存逻辑",
    "topics": [
        {
            "title": "# 医生给1岁娃开小众药被AI提醒慎用",
            "heat": "778,179",
            "category_color": "#4facfe", # Tech Blue
            "category_bg": "linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)",
        ...
        },
        {
            ...
            "heat": "1,076,529",
            "category_color": "#FF4757", # Social Red
            "category_bg": "linear-gradient(135deg, #FF6B6B 0%, #FF8E53 100%)",
        ...
        },
        {
            "title": "# 用大寒节气打开冬日中国之美",
            "heat": "598,206",
            "category_color": "#a29bfe", # Culture Purple
            "category_bg": "linear-gradient(135deg, #667eea 0%, #764ba2 100%)",
...
<head>
    ...
    <title>{report_data['report_title']} - {report_data['date']}</title>
    <link
...
<body>
    <div class="container">
        <header>
            <div class="date-chip">REPORT · {report_data['date']}</div>
            <h1>{report_data['report_title']}</h1>
            <p class="subtitle">{report_data['subtitle']}</p>
        </header>
        ...
        <div class="topic-card">
            <div class="card-header">
                <div class="topic-identity">
                    <div class="category-tag" style="color: {topic['category_color']}; background: {topic['category_bg']};"></div>
...
html_template += f"""
        ...
        <footer>
            ...
            <p style="margin-top: 5px;">基于 {datetime.datetime.now().strftime("%Y/%m/%d %H:%M")} 实时动态生成</p>
        </footer>
    </div>
</body>
